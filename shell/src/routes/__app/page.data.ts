...
import type { BudgetProgress, SavingsGoal } from "@/mock";
...
type BudgetSummary = { rows: BudgetProgress[]; totalLimit: number; totalSpent: number };
const EMPTY_BUDGET: BudgetSummary = { rows: [], totalLimit: 0, totalSpent: 0 };
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const canBudgets = session.entitlements.includes("budgets");

  const [{ accounts, netWorth }, payees, security, budget, goals] = await Promise.all([
    loadAccountsList({ wealth: session.entitlements.includes("wealth") }),
    loadPayees(),
    loadSecurityOverview(),
    canBudgets ? loadBudgetProgress() : Promise.resolve(EMPTY_BUDGET),
    canBudgets ? loadSavingsGoals() : Promise.resolve([] as SavingsGoal[]),
  ]);
